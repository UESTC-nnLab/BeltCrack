...
class SwinTransformerLayer3D(nn.Module):
    ...
    def forward_part1(self, x, mask_matrix):
        B, D, H, W, C = x.shape
        # print("执行到了",x.shape)    # [4, 5, 64, 64, 128]
...
class SwinTransformerBlock3D(nn.Module):
    ...
    def reshape_frames(self, x, mode:int=0):
        if mode == 0:
            ...
            c_new = c // self.num_frames
            x = x.reshape(b, self.num_frames, c_new, h, w)
        elif mode == 1:
            b, t, c,  h, w = x.shape 
    ...
    def forward(self, x):
        # print(f"x shape begfore swin transformer {x.shape}") #[4, 640, 64, 64]
...
if __name__ == "__main__":
    net = SwinTransformerBlock3D(c1=128)
    input = torch.rand(4,128*5,64,64)
    out = net(input)
    print(out.shape)  # [4, 640, 64, 64]
