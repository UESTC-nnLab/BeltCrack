...
class GAL(nn.Module):
    def __init__(self, sync_bn=True, input_channels=128): 
    ...
    def forward(self, input):
        ...
        cat_feature_for_vertex = torch.cat((vertex, aggregated_edge), dim=1)  #[2, 256, 64, 64]
        update_vertex = self.vertex_update_func(
            cat_feature_for_vertex.permute(0, 2, 3, 1).reshape((-1, 2 * self.input_channels))
        ).reshape((B, H, W, self.input_channels // 2)).permute(0, 3, 1, 2)    #  [2,64,64,64]
...
if __name__ == "__main__":
    net = GAL(sync_bn=True, input_channels=128)
    a = torch.rand(4,128,64,64)
    out = net(a)
    print(out.shape) #[4, 128, 64, 64]