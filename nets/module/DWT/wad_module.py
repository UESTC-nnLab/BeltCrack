...
class wad_module(nn.Module):
    ...
    def forward(self, input):
        LL, LH, HL, _ = self.dwt(input)  #1,3 128,128
        ...
        x_high = self.softmax(torch.add(LH, HL)) #1,3,128,128
        # x_high = self.softmax(torch.add(LH, HL).sub(_))  # xyl 20221013 想减去噪声信息 HH
        AttMap = torch.mul(output, x_high) #1,3,128,128
...
if __name__ == '__main__':
    ...
    img = imageio.imread(r'./0.bmp')  # array (255,255,3)
