...
class seqDataset(Dataset):
    ...
    def get_data(self, index):
        ...
        for id in range(0, self.num_frame):
            img = Image.open(image_path +'%d.jpg' % max(image_id - id, 0))
            img = cvtColor(img)
            iw, ih = img.size
...
if __name__ == "__main__":
    train_dataset = seqDataset("/home/hjh/桌面/crackBaseannotations/D/train/coco.txt", 256, 5, 'train')  # 默认测试样例，实际与调用函数有关
